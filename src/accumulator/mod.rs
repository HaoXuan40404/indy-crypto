...
pub enum CredentialValue {
    Known { value: BigNumber },
    //Issuer and Prover know these
    Hidden { value: BigNumber },
...
pub struct CredentialValuesBuilder {
    attrs_values: BTreeMap<String, CredentialValue>,
    /* attr_name -> int representation of value */
}
...
mod test {
    ...
    use accumulator::issuer::mocks::{max_cred_num, revocation_key_public};
    ...
    fn demo_accumulator() {
        ...
        println!("{}", "Issuer初始化，限定证书个数，生成聚合器".green());
        let max_element_num = 5;
        println!("{}", "Issuer生成公私钥对".green());
        let (credentialRevocationPublicKey, credentialRevocationPrivateKey) = Issuer::new_revocation_keys().unwrap();
        print!("{}", "公钥credentialRevocationPublicKey = ".yellow());
        println!("{:?}", credentialRevocationPublicKey);
        print!("{}", "私钥credentialRevocationPrivateKey = ".yellow());
        println!("{:?}", credentialRevocationPrivateKey);
        pause();
        println!("{}", "Issuer初始化聚合器公私钥对、聚合值".green());
        let (mut accumulator_pub, accumulator_pri_key) = Issuer::newAccumulator(&credentialRevocationPublicKey, max_cred_num()).unwrap();
        ...
        println!("{}", "Prover使用Issuer公钥生成盲化因子".green());
        let blindedRevocationSecrets = Prover::generateBlindedRevocation(&credentialRevocationPublicKey).unwrap();
        print!("{}", "盲化因子blindedRevocationSecrets = ".yellow());
        println!("{:?}", blindedRevocationSecrets);
        pause();

        let mut rev_tails_generator = RevocationTailsGenerator::new(
            max_element_num,
            accumulator_pri_key.gamma.clone(),
            credentialRevocationPublicKey.g_dash.clone());
        let simple_tail_accessor = SimpleTailsAccessor::new(&mut rev_tails_generator).unwrap();
        ...
        print!("{}", "Prover revocationID= ".yellow());
        println!("{:?}", revocationID);
        let m2 = Issuer::_gen_credential_context(prover_id, Some(revocationID)).unwrap();
        print!("{}", "Prover 唯一身份标识 m2= ".yellow());
        println!("{:?}", m2);

        pause();
        println!("{}", "Issuer将Prover证据添加至聚合器，生成witness证明".green());
        let (mut nonRevocationCredentialSignature, revocationRegistryDelta) =
            Issuer::addToAccumulaor(revocationID, &m2, max_element_num, issuance_by_default, &blindedRevocationSecrets, &mut accumulator_pub,
                                    &accumulator_pri_key, &credentialRevocationPublicKey, &credentialRevocationPrivateKey, &simple_tail_accessor).unwrap();
        print!("{}", "未撤销证据的证明nonRevocationCredentialSignature= ".yellow());
        println!("{:?}", nonRevocationCredentialSignature);
        pause();

        println!("{}", "Issuer计算未撤销证据".green());
        let witness = Witness::new(revocationID, max_element_num, issuance_by_default, &revocationRegistryDelta.unwrap(), &simple_tail_accessor).unwrap();
        ...
        Prover::checkRevocationCredential(&mut nonRevocationCredentialSignature,
                                          &blindedRevocationSecrets, &credentialRevocationPublicKey, &accumulator_pub, &witness);


        Prover::storeNonRevocationCredential(&mut nonRevocationCredentialSignature, &blindedRevocationSecrets);

        let mut proof_builder = Prover::new_proof_builder().unwrap();
        println!("{}", "Prover对未撤销证明进行盲化".green());
        let nonRevocInitProof = Prover::initNonRevocationProof(&nonRevocationCredentialSignature, &accumulator_pub, &credentialRevocationPublicKey, &witness).unwrap();
        

        let proof_request_nonce = new_nonce().unwrap();

        let mut values: Vec<Vec<u8>> = Vec::new();
        values.extend_from_slice(&nonRevocInitProof.as_tau_list().unwrap());
        print!("{}", "tau_list= ".yellow());
        println!("{:?}", &nonRevocInitProof.as_tau_list().unwrap());    
        values.extend_from_slice(&nonRevocInitProof.as_c_list().unwrap());
        ...
        let nonRevocProof = Prover::finalizeNonRevocationProof(&nonRevocInitProof, &challenge).unwrap();
        print!("{}", "Prover盲化后的证据nonRevocProof= ".yellow());
        println!("{:?}", nonRevocProof);
        println!("{}", "至此，撤销证据生成流程结束".red());
        pause();

        println!("{}", "撤销证据验证流程开始".red());
        println!("{}", "Prover将盲化后的证据和证明发送给Verifier".green());
        println!("{}", "Verifier用盲化后的证据nonRevocProof、证明challenge、Issuer公钥、聚合值credentialRevocationPublicKey、accumulator_pub进行验证".green());
        let mut tau_list = Verifier::verifyNonRevocationProof(&credentialRevocationPublicKey,
                                                              &accumulator_pub, &challenge, &nonRevocProof).unwrap().as_slice().unwrap();
        ...
        print!("{}", "challenge= ".yellow());
        println!("{:?}", challenge);
        print!("{}", "c_hver= ".yellow());
        println!("{:?}", c_hver);
        // assert_eq!(&challenge,&c_hver);
        info!(target: "anoncreds_service", "Verifier verify proof -> done");
        println!("{}", "accumulator demo展示结束".green());
    }
}
