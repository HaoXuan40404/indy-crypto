...
impl Prover {
    ...
    /// # Example
    /// ```
    /// use indy_crypto::accumulator::prover::Prover;
    ///
    ...
    /// use indy_crypto::accumulator::issuer::Issuer;
    /// use indy_crypto::accumulator::prover::Prover;
    ///
    ...
    pub fn generateBlindedRevocation(credentialRevocationPublicKey: &CredentialRevocationPublicKey) -> Result<BlindedRevocationSecrets, IndyCryptoError> {
        trace!("Prover::_generate_blinded_revocation_credential_secrets: >>> r_pub_key: {:?}", credentialRevocationPublicKey);
        let s_prime = GroupOrderElement::new()?;
        let ur = credentialRevocationPublicKey.h2.mul(&s_prime)?;
        let blindedRevocationSecrets = BlindedRevocationSecrets { ur, s_prime };
        trace!("Prover::_generate_blinded_revocation_credential_secrets: <<< revocation_blinded_credential_secrets: {:?}", blindedRevocationSecrets);
        Ok(blindedRevocationSecrets)
    }
    ...
    //检验撤销凭证是否正确
    pub fn checkRevocationCredential(r_cred: &mut NonRevocationCredentialSignature,
                                     blindedRevocationSecrets: &BlindedRevocationSecrets,
                                     cred_rev_pub_key: &CredentialRevocationPublicKey,
                                     accumulatorPublic: &AccumulatorPublic,
                                     witness: &Witness) -> Result<(), IndyCryptoError> {
        let s_prime: GroupOrderElement = blindedRevocationSecrets.s_prime;

        trace!("Prover::_process_non_revocation_credential: >>> r_cred: {:?}, vr_prime: {:?}, cred_rev_pub_key: {:?},  rev_key_pub: {:?}",
               r_cred, s_prime, cred_rev_pub_key, accumulatorPublic);

        let r_cnxt_m2 = BigNumber::from_bytes(&r_cred.m2.to_bytes()?)?;
        r_cred.s_prime_prime = s_prime.add_mod(&r_cred.s_prime_prime)?;
        //Prover::_test_witness_signature(&r_cred, cred_rev_pub_key, accumulatorPublic, rev_reg, witness, &r_cnxt_m2)?;

        let z_calc = Pair::pair(&r_cred.witness_signature.g_i, &accumulatorPublic.accumulator)?
            .mul(&Pair::pair(&cred_rev_pub_key.g, &witness.omega)?.inverse()?)?;

        if z_calc != accumulatorPublic.accu_pub_key.z {
        ...
        let pair_h2 = Pair::pair(
            &cred_rev_pub_key.h0
                ...
                .add(&cred_rev_pub_key.h2.mul(&r_cred.s_prime_prime)?)?
                .add(&r_cred.g_i)?,
    ...
    pub fn storeNonRevocationCredential(nonRevocationCredentialSignature: &mut NonRevocationCredentialSignature,
                                        blindedRevocationSecrets: &BlindedRevocationSecrets) -> Result<(), IndyCryptoError> {
        let ref s_prime= blindedRevocationSecrets.s_prime;
        nonRevocationCredentialSignature.s_prime_prime = s_prime.add_mod(&nonRevocationCredentialSignature.s_prime_prime)?;
        Ok(())
    }
    ...
    fn _process_non_revocation_credential(r_cred: &mut NonRevocationCredentialSignature,
        ...
        let r_cnxt_m2 = BigNumber::from_bytes(&r_cred.m2.to_bytes()?)?;
        r_cred.s_prime_prime = vr_prime.add_mod(&r_cred.s_prime_prime)?;
    ...
    pub fn initNonRevocationProof(r_cred: &NonRevocationCredentialSignature,
                                  accumulatorPublic: &AccumulatorPublic,
                                  cred_rev_pub_key: &CredentialRevocationPublicKey,
        ...
        let tau_list = getTauList(&cred_rev_pub_key,
                                  &accumulatorPublic,
                                  &tau_list_params,
                                  &c_list)?;
    ...
    pub fn finalizeNonRevocationProof(init_proof: &NonRevocInitProof, c_h: &BigNumber) -> Result<NonRevocProof, IndyCryptoError> {
...
impl ProofBuilder {
    ...
    /// use indy_crypto::accumulator::issuer::Issuer;
    /// use indy_crypto::accumulator::prover::Prover;
    /// use indy_crypto::accumulator::verifier::Verifier;
    ///
    /// let mut credential_schema_builder = Issuer::new_credential_schema_builder().unwrap();
    /// credential_schema_builder.add_attr("sex").unwrap();
    /// let credential_schema = credential_schema_builder.finalize().unwrap();
...
"master_secret".to_string() => BigNumber::from_dec("4013850682121471572108494732681923882818824463486221403305684759463606521257843454944595738801258160965585302031329898063691848370284494122908692611653736561002522186660023387006").unwrap()
    ...
    pub fn revocation_credential() -> NonRevocationCredentialSignature {
        NonRevocationCredentialSignature {
            ...
            s_prime_prime: GroupOrderElement::from_string("82AD6D5A28057D 249C7DE575B04 C8DCE90C7B2A5E 131D9D72956B4 1DF0DB17").unwrap(),
