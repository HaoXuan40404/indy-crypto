...
impl Verifier {
    ...
    pub fn verifyNonRevocationProof(r_pub_key: &CredentialRevocationPublicKey,
                                    accumulatorPublic: &AccumulatorPublic,
                                    c_hash: &BigNumber, proof: &NonRevocProof) -> Result<NonRevocProofTauList, IndyCryptoError> {
        trace!("ProofVerifier::_verify_non_revocation_proof: >>> r_pub_key: {:?}, accumulatorPublic: {:?},  c_hash: {:?}",
               r_pub_key, accumulatorPublic, c_hash);

        let ch_num_z = bignum_to_group_element(&c_hash)?;

        let t_hat_expected_values = createTauListExpectedValues(r_pub_key, accumulatorPublic, &proof.c_list)?;
        let t_hat_calc_values =
            createTauListValues(&r_pub_key, accumulatorPublic, &proof.x_list, &proof.c_list)?;
