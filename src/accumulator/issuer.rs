...
impl Issuer {
    ...
    /// # Example
    /// ```
    /// use indy_crypto::accumulator::{new_nonce, SimpleTailsAccessor};
    /// use indy_crypto::accumulator::issuer::Issuer;
    /// use indy_crypto::accumulator::prover::Prover;
    ///
    /// let mut credential_schema_builder = Issuer::new_credential_schema_builder().unwrap();
    ...
    pub fn newAccumulator(cred_rev_pub_key: &CredentialRevocationPublicKey,
    ...
    pub fn addToAccumulaor(rev_idx: u32,
                           m2: &BigNumber,
                           maxElementNum: u32,
                           issuance_by_default: bool,
                           blindedRevocationSecrets: &BlindedRevocationSecrets,
                           accumulatorPublic: &mut AccumulatorPublic,
                           accumulatorPrivateKey: &AccumulatorPrivateKey,
                           credentialRevocationPublicKey: &CredentialRevocationPublicKey,
                           credentialRevocationPrivateKey: &CredentialRevocationPrivateKey,
                           rev_tails_accessor: &RevocationTailsAccessor)
                           -> Result<(NonRevocationCredentialSignature, Option<RevocationRegistryDelta>), IndyCryptoError> {
        ...
        let g_i = {
            let i_bytes = transform_u32_to_array_of_u8(rev_idx);
            let mut pow = GroupOrderElement::from_bytes(&i_bytes)?;
            pow = accumulatorPrivateKey.gamma.pow_mod(&pow)?;
            credentialRevocationPublicKey.g.mul(&pow)?
        };

        let ur = blindedRevocationSecrets.ur;

        let sigma =
            credentialRevocationPublicKey.h0.add(&credentialRevocationPublicKey.h1.mul(&m2)?)?
                .add(&ur)?
                .add(&g_i)?
                .add(&credentialRevocationPublicKey.h2.mul(&s_prime_prime)?)?
                .mul(&credentialRevocationPrivateKey.x.add_mod(&c)?.inverse()?)?;


        let sigma_i = credentialRevocationPublicKey.g_dash
            .mul(&credentialRevocationPrivateKey.sk
                .add_mod(&accumulatorPrivateKey.gamma
                    .pow_mod(&GroupOrderElement::from_bytes(&transform_u32_to_array_of_u8(rev_idx))?)?)?
                .inverse()?)?;
        let u_i = credentialRevocationPublicKey.u
            .mul(&accumulatorPrivateKey.gamma
                .pow_mod(&GroupOrderElement::from_bytes(&transform_u32_to_array_of_u8(rev_idx))?)?)?;

        let index = Issuer::_get_index(maxElementNum, rev_idx);

        let rev_reg_delta = if issuance_by_default {
            None
        } else {
            let prev_acc = accumulatorPublic.accumulator.clone();

            rev_tails_accessor.access_tail(index, &mut |tail| {
                accumulatorPublic.accumulator = accumulatorPublic.accumulator.add(tail).unwrap();
            })?;

            Some(RevocationRegistryDelta {
                prev_accum: Some(prev_acc),
                accum: accumulatorPublic.accumulator.clone(),
                issued: hashset![rev_idx],
                revoked: HashSet::new(),
            })
        };
...
mod tests {
    use super::*;
    use accumulator::issuer::{Issuer, mocks};
    use accumulator::helpers::MockHelper;
...
pub mod mocks {
    ...
    pub fn revocation_credential() -> NonRevocationCredentialSignature {
        NonRevocationCredentialSignature {
            ...
            s_prime_prime: GroupOrderElement::from_string("22901ECFBD8CAC21E4A041949CCAF01EDC1D555C1293FBD47D9C315785FAC643").unwrap(),
