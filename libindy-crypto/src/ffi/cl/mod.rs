...
#[no_mangle]
pub extern fn cl_tails_generator_next(rev_tails_generator: *const c_void,
                                                  tail_p: *mut *const c_void) -> ErrorCode {
    trace!("cl_tails_generator_next: >>> rev_tails_generator: {:?}, tail_p {:?}",
    ...
    let res = match rev_tails_generator.next() {
        Ok(tail) => {
            unsafe {
                ...
                trace!("cl_tails_generator_next: *tail_p: {:?}", *tail_p);
            }
            ErrorCode::Success
        }
    ...
    trace!("cl_tails_generator_next: <<< {:?}", res);
    res
}

#[no_mangle]
pub extern fn cl_tails_generator_count(rev_tails_generator: *const c_void,
                                                   count_p: *mut u32) -> ErrorCode {
    trace!("cl_tails_generator_count: >>> rev_tails_generator: {:?}, count_p {:?}",
    ...
    unsafe {
        *count_p = cnt;
        trace!("cl_tails_generator_count: *count_p: {:?}", *count_p);
    }
    let res = ErrorCode::Success;


    trace!("cl_tails_generator_count: <<< {:?}", res);
    res
}

#[no_mangle]
pub extern fn cl_tail_free(tail: *const c_void) -> ErrorCode {
    trace!("cl_tail_free: >>> tail: {:?}", tail);

    check_useful_c_ptr!(tail, ErrorCode::CommonInvalidParam1);

    let tail = unsafe { Box::from_raw(tail as *mut Tail); };
    trace!("cl_tail_free: entity: tail: {:?}", tail);

    let res = ErrorCode::Success;

    trace!("cl_tail_free: <<< res: {:?}", res);
    res
}

#[no_mangle]
pub extern fn cl_witness_new(rev_idx: u32,
                                         max_cred_num: u32,
    ...
    trace!("cl_witness_new: >>> rev_idx: {:?}, max_cred_num {}, issuance_by_default {}, rev_reg_delta {:?}, ctx_tails {:?}, take_tail {:?}, \
        ...
        Ok(witness) => {
            unsafe {
                *witness_p = Box::into_raw(Box::new(witness)) as *const c_void;
                trace!("cl_witness_new: *witness_p: {:?}", *witness_p);
            }
            ErrorCode::Success
        }
        Err(err) => err.to_error_code()
    ...
    trace!("cl_witness_new: <<< res: {:?}", res);
    res
}

#[no_mangle]
pub extern fn cl_witness_update(rev_idx: u32,
                                            max_cred_num: u32,
                                            rev_reg_delta: *const c_void,
                                            witness: *mut c_void,
                                            ctx_tails: *const c_void,
    ...
    trace!("cl_witness_update: >>> rev_idx: {:?}, max_cred_num {}, rev_reg_delta {:?}, ctx_tails {:?}, take_tail {:?}, put_tail {:?}, witness {:?}",
    ...
    trace!("cl_witness_update: <<< res: {:?}", res);
    res
}

#[no_mangle]
pub extern fn cl_witness_free(witness: *const c_void) -> ErrorCode {
    trace!("cl_witness_free: >>> witness: {:?}", witness);

    check_useful_c_ptr!(witness, ErrorCode::CommonInvalidParam1);

    let witness = unsafe { Box::from_raw(witness as *mut Witness); };
    trace!("cl_witness_free: entity: witness: {:?}", witness);

    let res = ErrorCode::Success;

    trace!("cl_witness_free: <<< res: {:?}", res);
    res
}
...
pub mod mocks {
    ...
    pub fn _witness(rev_reg_delta: *const c_void, get_storage_ctx: *const c_void) -> *const c_void {
        ...
        let err_code = cl_witness_new(rev_idx,
                                                  max_cred_num,
                                                  false,
                                                  rev_reg_delta,
                                                  get_storage_ctx,
    ...
    pub fn _free_witness(witness: *const c_void) {
        let err_code = cl_witness_free(witness);
        assert_eq!(err_code, ErrorCode::Success);
    }
    ...
    impl FFISimpleTailStorage {
        pub fn new(rev_tails_generator: *const c_void) -> Self {
            ...
            let mut cnt = 0u32;
            let res = cl_tails_generator_count(rev_tails_generator, &mut cnt);
            assert_eq!(res, ErrorCode::Success);
            for _ in 0..cnt {
                let mut tail = ptr::null();
                let res = cl_tails_generator_next(rev_tails_generator, &mut tail);
                assert_eq!(res, ErrorCode::Success);
                tails.push(tail);
            }
            Self {
